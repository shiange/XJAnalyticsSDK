...
public class NetworkUtils {
    ...
    public static String networkType(Context context) {
        try {
            // 检测权限
            if (!XJDataUtils.checkHasPermission(context, Manifest.permission.ACCESS_NETWORK_STATE)) {
                return "NULL";
            }
    ...
    private static int toNetworkType(String networkType) {
        if ("NULL".equals(networkType)) {
            return XJDataAPI.NetworkType.TYPE_ALL;
        } else if ("WIFI".equals(networkType)) {
            return XJDataAPI.NetworkType.TYPE_WIFI;
        } else if ("2G".equals(networkType)) {
            return XJDataAPI.NetworkType.TYPE_2G;
        } else if ("3G".equals(networkType)) {
            return XJDataAPI.NetworkType.TYPE_3G;
        } else if ("4G".equals(networkType)) {
            return XJDataAPI.NetworkType.TYPE_4G;
        } else if ("5G".equals(networkType)) {
            return XJDataAPI.NetworkType.TYPE_5G;
        }
        return XJDataAPI.NetworkType.TYPE_ALL;
    }
    ...
    private static String mobileNetworkType(Context context, TelephonyManager telephonyManager, ConnectivityManager connectivityManager) {
        ...
        if (telephonyManager != null) {
            if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.R
                    && (XJDataUtils.checkHasPermission(context, Manifest.permission.READ_PHONE_STATE) || telephonyManager.hasCarrierPrivileges())) {
