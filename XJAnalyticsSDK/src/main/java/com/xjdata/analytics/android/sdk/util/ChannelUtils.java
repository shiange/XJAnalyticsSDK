...
import static com.xjdata.analytics.android.sdk.util.XJDataUtils.getSharedPreferences;
...
public class ChannelUtils {
    ...
    public static String getDeviceInfo(Context mContext, String androidId, String oaid) {
        return String.format("android_id=%s##imei=%s##imei_old=%s##imei_slot1=%s##imei_slot2=%s##imei_meid=%s##mac=%s##oaid=%s",
                androidId,
                XJDataUtils.getIMEI(mContext),
                XJDataUtils.getIMEIOld(mContext),
                XJDataUtils.getSlot(mContext, 0),
                XJDataUtils.getSlot(mContext, 1),
                XJDataUtils.getMEID(mContext),
                XJDataUtils.getMacAddress(mContext),
                oaid);
    }
    ...
    public static boolean isGetDeviceInfo(Context context, String androidId, String oaid) {
        try {
            return !TextUtils.isEmpty(androidId) ||
                    ...
                    !TextUtils.isEmpty(XJDataUtils.getIMEI(context)) ||
                    !TextUtils.isEmpty(XJDataUtils.getIMEIOld(context)) ||
                    !TextUtils.isEmpty(XJDataUtils.getSlot(context, 0)) ||
                    !TextUtils.isEmpty(XJDataUtils.getSlot(context, 1)) ||
                    !TextUtils.isEmpty(XJDataUtils.getMEID(context));
        } catch (Exception e) {
            XJLog.printStackTrace(e);
        }
        return false;
    ...
    public static boolean checkDeviceInfo(Context context, String deviceInfo) {
        ...
        return (deviceMaps.containsKey("oaid")//防止都为 null 返回 true
                       ...
                       (deviceMaps.containsKey("imei") &&
                                TextUtils.equals(deviceMaps.get("imei"), XJDataUtils.getIMEI(context))) ||
                       (deviceMaps.containsKey("imei_old") &&
                                TextUtils.equals(deviceMaps.get("imei_old"),XJDataUtils.getIMEIOld(context))) ||
                       (deviceMaps.containsKey("imei_slot1") &&
                                TextUtils.equals(deviceMaps.get("imei_slot1"), XJDataUtils.getSlot(context, 0))) ||
                       (deviceMaps.containsKey("imei_slot2") &&
                                TextUtils.equals(deviceMaps.get("imei_slot2"), XJDataUtils.getSlot(context, 1))) ||
                       (deviceMaps.containsKey("imei_meid") &&
                                TextUtils.equals(deviceMaps.get("imei_meid"), XJDataUtils.getMEID(context))) ||
                       (deviceMaps.containsKey("android_id") &&
                                TextUtils.equals(deviceMaps.get("android_id"), XJDataUtils.getAndroidID(context))) ||
                       (deviceMaps.containsKey("mac") &&
                                TextUtils.equals(deviceMaps.get("mac"), XJDataUtils.getMacAddress(context)));
    }
}
