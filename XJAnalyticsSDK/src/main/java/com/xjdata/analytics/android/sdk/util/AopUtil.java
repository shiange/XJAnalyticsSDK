...
import com.xjdata.analytics.android.sdk.XJDataFragmentTitle;
import com.xjdata.analytics.android.sdk.visual.model.ViewNode;
...
public class AopUtil {
    ...
    public static void getScreenNameAndTitleFromFragment(JSONObject properties, Object fragment, Activity activity) {
        try {
            ...
            if (fragment instanceof XJAutoTracker) {
                ...
                JSONObject trackProperties = XJAutoTracker.getTrackProperties();
                if (trackProperties != null) {
                    ...
                    XJDataUtils.mergeJSONObject(trackProperties, properties);
                }
            }

            if (TextUtils.isEmpty(title) && fragment.getClass().isAnnotationPresent(XJDataFragmentTitle.class)) {
    ...
    public static JSONObject buildTitleAndScreenName(Activity activity) {
        ...
        try {
            ...
            if (activity instanceof XJAutoTracker) {
                ...
                JSONObject trackProperties = XJAutoTracker.getTrackProperties();
                if (trackProperties != null) {
                    XJDataUtils.mergeJSONObject(trackProperties, propertyJSON);
                }
            }
        } catch (Exception ex) {
            XJLog.printStackTrace(ex);
    ...
    public static boolean isViewIgnored(Class viewType) {
        try {
            ...
            List<Class> mIgnoredViewTypeList = XJDataAPI.sharedInstance().getIgnoredViewTypeList();
        ...
        try {
            if (activity != null) {
                try {
                    ...
                    if (Build.VERSION.SDK_INT >= 11) {
                        String toolbarTitle = XJDataUtils.getToolbarTitle(activity);
    ...
    /**
     * properties 注入点击事件信息
     * 属性的优先级为：预置属性低于 {@link XJAutoTracker#getTrackProperties()} 低于
     * {@link XJDataAPI#setViewProperties(View, JSONObject)} 低于
     * {@link XJDataAPI#trackViewAppClick(View, JSONObject)}
     *
     * @param view 点击的 view
    ...
    public static boolean injectClickInfo(View view, JSONObject properties, boolean isFromUser) {
        ...
        try {
            ...
            if (activity != null) {
                XJDataUtils.mergeJSONObject(AopUtil.buildTitleAndScreenName(activity), eventJson);
            }
    ...
    public static void addViewPathProperties(Activity activity, View view, JSONObject properties) {
        try {
            ...
            if ((XJDataAPI.sharedInstance().isHeatMapEnabled() || XJDataAPI.sharedInstance().isVisualizedAutoTrackEnabled())
                    && (XJDataAPI.sharedInstance().isHeatMapActivity(activity.getClass()) || XJDataAPI.sharedInstance().isVisualizedAutoTrackActivity(activity.getClass()))) {
            ...
            if (viewNode != null) {
                if (!TextUtils.isEmpty(viewNode.getViewPath())) {
                    if ((XJDataAPI.sharedInstance().isVisualizedAutoTrackEnabled() && XJDataAPI.sharedInstance().isVisualizedAutoTrackActivity(activity.getClass()))
                    || (XJDataAPI.sharedInstance().isHeatMapEnabled() && XJDataAPI.sharedInstance().isHeatMapActivity(activity.getClass()))) {
