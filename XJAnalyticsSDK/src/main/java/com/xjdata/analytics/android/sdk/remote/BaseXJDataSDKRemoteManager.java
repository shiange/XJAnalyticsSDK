...
public abstract class BaseXJDataSDKRemoteManager {

    protected static final String TAG = "SA.SensorsDataSDKRemoteConfigBase";
    protected Context mContext;
    protected XJConfigOptions mSAConfigOptions;
    protected XJDataEncrypt mXJDataEncrypt;
    protected boolean mDisableDefaultRemoteConfig;

    protected static XJDataSDKRemoteConfig mSDKRemoteConfig;
    protected XJDataAPI mSensorsDataAPI;

    protected BaseXJDataSDKRemoteManager(XJDataAPI sensorsDataAPI) {
        ...
        this.mXJDataEncrypt = sensorsDataAPI.getSensorsDataEncrypt();
    ...
    protected abstract void setSDKRemoteConfig(XJDataSDKRemoteConfig sdkRemoteConfig);
            ...
            try {
                ...
                for (int i = 0; i < size; i++) {
                    if (eventName.equals(mSDKRemoteConfig.getEventBlacklist().get(i))) {
                        XJLog.i(TAG, "remote config: " + eventName + " is ignored by remote config");
                        return true;
                    }
                }
     ...
     */
    protected XJDataSDKRemoteConfig toSDKRemoteConfig(String config) {
        XJDataSDKRemoteConfig sdkRemoteConfig = new XJDataSDKRemoteConfig();
        try {
            if (!TextUtils.isEmpty(config)) {
                ...
                if (!TextUtils.isEmpty(configs)) {
                    ...
                    if (keyObject != null) {
                        if (keyObject.has("key_ec") && XJDataEncrypt.isECEncrypt()) {
    ...
    public Boolean isAutoTrackEventTypeIgnored(int autoTrackEventType) {
        if (mSDKRemoteConfig != null) {
            if (mSDKRemoteConfig.getAutoTrackMode() != XJDataSDKRemoteConfig.REMOTE_EVENT_TYPE_NO_USE) {
    ...
    public Boolean isAutoTrackEnabled() {
        if (mSDKRemoteConfig != null) {
            if (mSDKRemoteConfig.getAutoTrackMode() == 0) {
                XJLog.i(TAG, "remote config: AutoTrackMode is closing by remote config");
                return false;
    ...
    protected String buildRemoteUrl(boolean enableConfigV) {
        ...
        if (!TextUtils.isEmpty(configOptionsRemoteUrl)
            ...
            XJLog.i(TAG, "SAConfigOptions remote url is " + remoteUrl);
            ...
            XJLog.i(TAG, "SensorsDataAPI remote url is " + remoteUrl);
        } else {
            XJLog.i(TAG, String.format(Locale.CHINA, "ServerUlr: %s, SAConfigOptions remote url: %s",
                    serverUlr, configOptionsRemoteUrl));
            XJLog.i(TAG, "Remote config url verification failed");
            return null;
        }

        //再次检查是否应该在请求中带 v，比如在禁止分散请求的情况下，SDK 升级了或者公钥为空，此时应该不带 v
        if (configV && (XJDataUtils.checkVersionIsNew(mContext, mSensorsDataAPI.getSDKVersion()) ||
                (mXJDataEncrypt != null && mXJDataEncrypt.isPublicSecretKeyNull()))) {
            configV = false;
        }
        ...
        if (!TextUtils.isEmpty(remoteUrl) && configV) {
            ...
            XJDataSDKRemoteConfig SDKRemoteConfig = mSDKRemoteConfig;
            if (SDKRemoteConfig != null) {
                oldVersion = SDKRemoteConfig.getOldVersion();
                newVersion = SDKRemoteConfig.getNewVersion();
                XJLog.i(TAG, "The current config: " + SDKRemoteConfig.toString());
            }
        ...
        XJLog.i(TAG, "Android remote config url is " + remoteUrl);
        return remoteUrl;
    }
