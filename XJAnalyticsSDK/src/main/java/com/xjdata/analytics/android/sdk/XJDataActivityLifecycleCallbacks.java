...
import com.xjdata.analytics.android.sdk.deeplink.DeepLinkManager;
...
import static com.xjdata.analytics.android.sdk.deeplink.DeepLinkManager.IS_ANALYTICS_DEEPLINK;


@TargetApi(Build.VERSION_CODES.ICE_CREAM_SANDWICH)
class XJDataActivityLifecycleCallbacks implements Application.ActivityLifecycleCallbacks {
    ...
    private final XJDataAPI mSensorsDataInstance;
    ...
    XJDataActivityLifecycleCallbacks(XJDataAPI instance, PersistentFirstStart firstStart,
        ...
        try {
            ...
            lib_version = XJDataAPI.VERSION;
        } catch (final Exception e) {
            XJLog.i(TAG, "Exception getting version name = ", e);
        }
        initHandler();
    }
    ...
    public void onActivityCreated(Activity activity, Bundle bundle) {
        XJDataUtils.handleSchemeUrl(activity, activity.getIntent());
    }
    ...
    public void onActivityResumed(final Activity activity) {
        try {
            ...
            if (mSensorsDataInstance.isAutoTrackEnabled() && !mSensorsDataInstance.isActivityAutoTrackAppViewScreenIgnored(activity.getClass())
                    && !mSensorsDataInstance.isAutoTrackEventTypeIgnored(XJDataAPI.AutoTrackEventType.APP_VIEW_SCREEN)) {
                JSONObject properties = new JSONObject();
                XJDataUtils.mergeJSONObject(activityProperty, properties);
                if (activity instanceof XJAutoTracker) {
                    XJAutoTracker XJAutoTracker = (XJAutoTracker) activity;
                ...
                mSensorsDataInstance.trackViewScreen(XJDataUtils.getScreenUrl(activity), eventProperties);
            }
        } catch (Exception e) {
        ...
        try {
            ...
            if (mStartActivityCount == 1) {
                if (mSensorsDataInstance.isSaveDeepLinkInfo()) {// 保存 utm 信息时,在 endData 中合并保存的 latestUtm 信息。
                    XJDataUtils.mergeJSONObject(ChannelUtils.getLatestUtmProperties(), endDataProperty);
                }
...
//                    mSensorsDataInstance.resumeTrackTaskThread();
                    ...
                    try {
                        if (mSensorsDataInstance.isAutoTrackEnabled() && !mSensorsDataInstance.isAutoTrackEventTypeIgnored(XJDataAPI.AutoTrackEventType.APP_START)) {
                            if (firstStart) {
                            ...
                            if (mDeepLinkProperty != null) {
                                XJDataUtils.mergeJSONObject(mDeepLinkProperty, properties);
    ...
    private void generateAppEndData(long messageTime, long endElapsedTime) {
        try {
            // 同意合规时进行打点记录
            if (XJDataAPI.getConfigOptions().isDataCollectEnable) {
    ...
    private boolean isSessionTimeOut() {
        ...
        XJLog.d(TAG, "SessionTimeOut:" + sessionTimeOut);
