...
abstract class DataOperation {
    ...
    int deleteDataLowMemory(Uri uri) {
        if (belowMemThreshold()) {
            XJLog.i(TAG, "There is not enough space left on the device to store events, so will delete 100 oldest events");
    ...
    private long getMaxCacheSize(Context context) {
        try {
            return XJDataAPI.sharedInstance(context).getMaxCacheSize();
        } catch (Exception e) {
