...
/* package */ public class AppWebViewInterface {
    ...
    public String sensorsdata_call_app() {
        try {
            ...
            String loginId = XJDataAPI.sharedInstance(mContext).getLoginId();
            if (!TextUtils.isEmpty(loginId)) {
                properties.put("distinct_id", loginId);
                properties.put("is_login", true);
            } else {
                properties.put("distinct_id", XJDataAPI.sharedInstance(mContext).getAnonymousId());
    ...
    public void sensorsdata_track(String event) {
        try {
            XJDataAPI.sharedInstance(mContext).trackEventFromH5(event, enableVerify);
        } catch (Exception e) {
    ...
    public boolean sensorsdata_verify(String event) {
        try {
            ...
            return XJDataAPI.sharedInstance(mContext)._trackEventFromH5(event);
        } catch (Exception e) {
            XJLog.printStackTrace(e);
            return false;
        }
    ...
    public String sensorsdata_get_server_url() {
        return XJDataAPI.sharedInstance().getConfigOptions().isAutoTrackWebView ? XJDataAPI.sharedInstance().getServerUrl() : "";
    }
    ...
    public boolean sensorsdata_visual_verify(String event) {
        try {
            ...
            if (!TextUtils.isEmpty(serverUrl)) {
                if (!(new ServerUrl(serverUrl).check(new ServerUrl(XJDataAPI.sharedInstance().getServerUrl())))) {
                    return false;
                }
                return true;
            }
    ...
    public void sensorsdata_js_call_app(final String content) {
        try {
            if (mWebView != null) {
                XJDataAPI.sharedInstance().handleJsMessage(mWebView,content);
            }
        } catch (Exception e) {
            XJLog.printStackTrace(e);
        }
