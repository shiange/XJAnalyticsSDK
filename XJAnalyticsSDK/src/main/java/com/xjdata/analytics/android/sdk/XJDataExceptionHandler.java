...
class XJDataExceptionHandler implements Thread.UncaughtExceptionHandler {
    private static final int SLEEP_TIMEOUT_MS = 500;

    private static XJDataExceptionHandler sInstance;
    private Thread.UncaughtExceptionHandler mDefaultExceptionHandler;
    private static boolean isTrackCrash = false;

    private XJDataExceptionHandler() {
    ...
    synchronized static void init() {
        if (sInstance == null) {
            sInstance = new XJDataExceptionHandler();
        }
    }
        ...
        try {
            if (isTrackCrash) {
                try {
                    ...
                    XJDataAPI.sharedInstance().trackEvent(EventType.TRACK, "AppCrashed", messageProp, null);
                } catch (Exception ex) {
                    XJLog.printStackTrace(ex);
                }
            }
            ...
            if (XJDataAPI.sharedInstance().isMultiProcessFlushData()) {
            ...
            XJDataAPI.sharedInstance().flush();

            try {
                Thread.sleep(SLEEP_TIMEOUT_MS);
            } catch (InterruptedException e1) {
                XJLog.printStackTrace(e1);
            }
