...
import com.xjdata.analytics.android.sdk.util.XJDisplayUtil;
...
class DebugModeSelectDialog extends Dialog implements View.OnClickListener {
    ...
    private XJDataAPI.DebugMode currentDebugMode;

    DebugModeSelectDialog(Context context, XJDataAPI.DebugMode debugMode) {
        super(context);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (window != null) {
            ...
            p.width = XJDisplayUtil.dip2px(getContext(), 270);
            p.height = XJDisplayUtil.dip2px(getContext(), 240);
            window.setAttributes(p);
            //设置弹框圆角
            GradientDrawable bg = new GradientDrawable();
            bg.setShape(GradientDrawable.RECTANGLE);
            bg.setColor(Color.WHITE);
            bg.setCornerRadius(XJDisplayUtil.dip2px(getContext(), 7));
    ...
    private void initView() {
        ...
        String msg = "调试模式已关闭";
        if (currentDebugMode == XJDataAPI.DebugMode.DEBUG_ONLY) {
            msg = "当前为 调试模式（不导入数据）";
        } else if (currentDebugMode == XJDataAPI.DebugMode.DEBUG_AND_TRACK) {
            msg = "当前为 测试模式（导入数据）";
        }
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.sensors_analytics_debug_mode_track) {
            onDebugModeDialogClickListener.setDebugMode(this, XJDataAPI.DebugMode.DEBUG_AND_TRACK);
        } else if (id == R.id.sensors_analytics_debug_mode_only) {
            onDebugModeDialogClickListener.setDebugMode(this, XJDataAPI.DebugMode.DEBUG_ONLY);
    ...
    interface OnDebugModeViewClickListener {
        ...
        void setDebugMode(Dialog dialog, XJDataAPI.DebugMode debugMode);
    }
}
