...
import com.xjdata.analytics.android.sdk.BuildConfig;
...
import com.xjdata.analytics.android.sdk.visual.snap.EditProtocol;
import com.xjdata.analytics.android.sdk.visual.snap.EditState;
import com.xjdata.analytics.android.sdk.visual.snap.ResourceIds;
import com.xjdata.analytics.android.sdk.visual.snap.ResourceReader;
    ...
    private class ViewCrawlerHandler extends Handler {
        ...
        private void sendSnapshot(JSONObject message) {
            ...
            try {
                ...
                if (null == mSnapshot) {
                    XJLog.i(TAG, "Snapshot should be initialize at first calling.");
                    return;
                }
            } catch (final JSONException e) {
                XJLog.i(TAG, "Payload with snapshot config required with snapshot request", e);
                return;
            } catch (final EditProtocol.BadInstructionsException e) {
                XJLog.i(TAG, "VisualizedAutoTrack server sent malformed message with snapshot request", e);
                return;
            }
            ...
            try {
                ...
                XJLog.i(TAG, "page_name： " + pageName);
                ...
                XJLog.i(TAG, "Can't write snapshot request to server", e);
            } finally {
                try {
                    writer.close();
                } catch (final IOException e) {
                    XJLog.i(TAG, "Can't close writer.", e);
                }
            }
            onSnapFinished(info);
            postSnapshot(out);
