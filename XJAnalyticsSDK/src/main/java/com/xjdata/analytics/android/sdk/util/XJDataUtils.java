...
import com.xjdata.analytics.android.sdk.XJDataAutoTrackAppViewScreenUrl;
...
public final class XJDataUtils {
    ...
    private static final String TAG = "XJ.XJDataUtils";
    ...
    public static String getCarrier(Context context) {
        try {
            if (XJDataUtils.checkHasPermission(context, Manifest.permission.READ_PHONE_STATE)) {
                try {
            ...
            if (TextUtils.isEmpty(userAgent)) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                    try {
                        ...
                        XJLog.i(TAG, "WebSettings NoSuchMethod: getDefaultUserAgent");
                    }
                }
    ...
    public static boolean checkHasPermission(Context context, String permission) {
        try {
            ...
            if (result != PackageManager.PERMISSION_GRANTED) {
                XJLog.i(TAG, "You can fix this by adding the following to your AndroidManifest.xml file:\n"
            ...
            if (tm != null) {
                if (Build.VERSION.SDK_INT > Build.VERSION_CODES.P) {
                    if (tm.hasCarrierPrivileges()) {
                        ...
                        XJLog.d(TAG, "Can not get IMEI info.");
                    }
    ...
    private static boolean hasReadPhoneStatePermission(Context context) {
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.P) {
            if (!XJDataUtils.checkHasPermission(context, Manifest.permission.READ_PRECISE_PHONE_STATE)) {
                XJLog.i(TAG, "Don't have permission android.permission.READ_PRECISE_PHONE_STATE,getDeviceID failed");
                return false;
            }
        } else if (!XJDataUtils.checkHasPermission(context, Manifest.permission.READ_PHONE_STATE)) {
            XJLog.i(TAG, "Don't have permission android.permission.READ_PHONE_STATE,getDeviceID failed");
            return false;
        }
        return true;
    }
    ...
    public static String getScreenUrl(Object object) {
        ...
        try {
            if (object instanceof XJAutoTracker) {
                XJAutoTracker XJAutoTracker = (XJAutoTracker) object;
                screenUrl = XJAutoTracker.getScreenUrl();
            } else {
                XJDataAutoTrackAppViewScreenUrl autoTrackAppViewScreenUrl = object.getClass().getAnnotation(XJDataAutoTrackAppViewScreenUrl.class);
    ...
    public static void handleSchemeUrl(Activity activity, Intent intent) {
        XJSchemeHelper.handleSchemeUrl(activity, intent);
    }
}
